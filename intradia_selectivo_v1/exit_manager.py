from intradia_selectivo_v1.config import BE_TRIGGER_PCT, MAX_HOLD_MINUTES, TRAILING_ACTIVATION_PCT, TRAILING_DISTANCE_ATR, FAST_FAIL_MINUTES, MIN_PROGRESS_FOR_HOLD, SCRATCH_EXIT_PCT


def manage_exit(trade, current_price, current_candle, minutes_elapsed, rsi_15m):
    ...
    if trade['direction'] == 'LONG':
        ...
        if progress >= TRAILING_ACTIVATION_PCT:
            ...
            trade['sl'] = max(trade['sl'], trade['max_price'] - (trade['atr'] * TRAILING_DISTANCE_ATR))
        ...
        if progress >= 0.65 and retrace >= 0.4:
            return current_price, 'GIVEBACK_EXIT', True
        if minutes_elapsed >= 240 and retrace >= 0.45 and rsi_15m < 52:
        ...
        if progress >= TRAILING_ACTIVATION_PCT:
            trade['trailing_active'] = True
            trade['sl'] = min(trade['sl'], trade['min_price'] + (trade['atr'] * TRAILING_DISTANCE_ATR))
        retrace = (current_price - trade['min_price']) / max(trade['entry'] - trade['min_price'], 1e-9) if trade['min_price'] < trade['entry'] else 0.0
        if minutes_elapsed >= FAST_FAIL_MINUTES and progress < MIN_PROGRESS_FOR_HOLD and current_price >= trade['entry'] * (1 - SCRATCH_EXIT_PCT):
            return current_price, 'NO_EXPANSION', True
        if progress >= 0.65 and retrace >= 0.4:
            return current_price, 'GIVEBACK_EXIT', True
        if minutes_elapsed >= 240 and retrace >= 0.45 and rsi_15m > 48:
            return current_price, 'MOMENTUM_DECAY', True
