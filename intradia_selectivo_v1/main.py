...
from intradia_selectivo_v1.config import EXCHANGE_ID, INITIAL_BALANCE, LOG_PATH, SYMBOLS, TF_CONTEXT, TF_ENTRY, SYMBOL_COOLDOWN_MINUTES, SYMBOL_REPEAT_LOSS_COOLDOWN_MINUTES
from intradia_selectivo_v1.db import init_db, insert_trade
from intradia_selectivo_v1.execution import build_trade
from intradia_selectivo_v1.exit_manager import manage_exit
from intradia_selectivo_v1.indicators import rsi
from intradia_selectivo_v1.risk import risk_checks
from intradia_selectivo_v1.scanner import scan_all_assets
from intradia_selectivo_v1.state import BotState
...
def fetch_ohlcv_safe(exchange, symbol, timeframe, limit=180, retries=5):
    delay = 1
...
def main():
    ...
    open_trade = None
    logging.info('Intradia Selectivo v1 started | dry_run=%s | exchange=%s', args.dry_run, EXCHANGE_ID)
    cycle = 0
    ...
    while True:
        ...
        symbol_to_candles_1h = {}
        for symbol in SYMBOLS:
            try:
                logging.info('fetch_start cycle=%s symbol=%s tf=%s/%s', cycle, symbol, TF_ENTRY, TF_CONTEXT)
                symbol_to_candles_15m[symbol] = fetch_ohlcv_safe(exchange, symbol, TF_ENTRY, limit=180)
                symbol_to_candles_1h[symbol] = fetch_ohlcv_safe(exchange, symbol, TF_CONTEXT, limit=180)
                logging.info('fetch_done cycle=%s symbol=%s candles_15m=%s candles_1h=%s', cycle, symbol, len(symbol_to_candles_15m[symbol]), len(symbol_to_candles_1h[symbol]))
            except Exception as exc:
                logging.warning('fetch failed %s: %s', symbol, exc)

        if open_trade is None:
            signal, diagnostics = scan_all_assets(symbol_to_candles_15m, symbol_to_candles_1h)
            if signal:
                ...
                if cooldown_until and now_ts < cooldown_until:
                    ...
                    logging.info('scan_cycle_no_signal cycle=%s symbols_ready=%s diagnostics=%s', cycle, len(symbol_to_candles_15m), diagnostics)
                else:
                    trade = build_trade(signal, state.balance)
                    if trade:
                        trade['opened_at'] = datetime.now(timezone.utc)
                        open_trade = trade
                        logging.info('OPEN %s %s entry=%s sl=%s tp=%s size=%s score=%.3f momentum=%.6f pullback=%.6f', trade['symbol'], trade['direction'], trade['entry'], trade['sl'], trade['tp'], trade['size'], trade['score'], trade['momentum_pct'], trade['pullback_pct'])
            else:
                logging.info('scan_cycle_no_signal cycle=%s symbols_ready=%s diagnostics=%s', cycle, len(symbol_to_candles_15m), diagnostics)
        else:
            candles = symbol_to_candles_15m.get(open_trade['symbol'])
            if candles:
                candle = candles[-1]
                current_price = candle[4]
                minutes_elapsed = (datetime.now(timezone.utc) - open_trade['opened_at']).total_seconds() / 60.0
                rsi_15m = rsi([c[4] for c in candles], 14)
                exit_price, exit_reason, closed = manage_exit(open_trade, current_price, candle, minutes_elapsed, rsi_15m)
                ...
                if closed:
                    ...
                    insert_trade((datetime.now(timezone.utc).isoformat(), open_trade['symbol'], open_trade['direction'], open_trade['entry'], exit_price, open_trade['size'], pnl, fee, exit_reason, state.balance, open_trade.get('score'), open_trade.get('momentum_pct'), open_trade.get('context_rsi'), open_trade.get('pullback_pct'), minutes_elapsed, open_trade.get('mfe'), open_trade.get('mae'), open_trade.get('peak_progress')))
