...
from reversion_scalp_v1_aggressive.config import EXCHANGE_ID, SYMBOLS, TF_CONTEXT, TF_ENTRY
from reversion_scalp_v1_aggressive.signal import detect_reversion_signal

REPORT_PATH = Path('reversion_scalp_v1_aggressive_signal_replay_report.txt')
CSV_PATH = Path('reversion_scalp_v1_aggressive_signal_replay.csv')
...
def apply_filter_variant(row, variant):
    ...
    if variant == 'score_055_070':
        return 0.55 <= (row.get('score') or 0) < 0.70
    if variant == 'score_055_080':
