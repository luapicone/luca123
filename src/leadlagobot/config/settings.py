...
class Settings(BaseModel):
    ...
    min_fill_ratio: float = float(os.getenv('MIN_FILL_RATIO', '0.20'))
    entry_threshold_pct: float = float(os.getenv('ENTRY_THRESHOLD_PCT', '0.10'))
    exit_threshold_pct: float = float(os.getenv('EXIT_THRESHOLD_PCT', '0.03'))
    min_quality_score: float = float(os.getenv('MIN_QUALITY_SCORE', '-0.05'))
    ...
    max_cancel_rate: float = float(os.getenv('MAX_CANCEL_RATE', '0.80'))
    top_pairs_limit: int = int(os.getenv('TOP_PAIRS_LIMIT', '20'))
    ranking_min_signals: int = int(os.getenv('RANKING_MIN_SIGNALS', '200'))
