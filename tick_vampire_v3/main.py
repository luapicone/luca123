...
from tick_vampire_v3.config import EXCHANGE, FEE_PCT, INITIAL_BALANCE, RSI_PERIOD, SESSIONS, SYMBOL, VOLUME_MA_PERIOD
    ...
    while True:
        ...
        if open_trade is None and len(closes) >= RSI_PERIOD + 1 and len(volumes) >= VOLUME_MA_PERIOD:
            ...
            if direction:
                ...
                open_trade = trade
                logging.info('OPEN %s size=%s entry=%s tp=%s sl=%s', direction, trade['size'], trade['entry'], trade['tp'], trade['sl'])
            else:
                skipped += 1
            ...
            if closed:
                ...
                logging.info('CLOSE %s pnl=%s reason=%s balance=%s', open_trade['direction'], round(pnl, 6), reason_exit, round(state.balance, 6))
                open_trade = None
